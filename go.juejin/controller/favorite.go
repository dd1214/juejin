...
func FavoriteAction(c *gin.Context) {
	...
	db.Select(&articles, "select ID, PlayUrl, CoverUrl, FavoriteCount, CommentCount, IsFavorite, Title from Video where ID=?", articleID)
	if articles == nil {
...
func FavoriteList(c *gin.Context) {
	...
	for _, article := range likeList {
		//获取视频列表
		var articles []dbArticle
		db.Select(&articles, "select ID, AuthorID, Url, FavoriteCount, CommentCount, IsFavorite, Title, PublishTime from Article where ID=?", article.ArticleID)
		if articles == nil {
			c.JSON(http.StatusOK, ArticleListResponse{
				Response: Response{
					StatusCode: 0,
